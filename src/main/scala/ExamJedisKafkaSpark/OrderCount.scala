...
object OrderCount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val broadcastRef = IPUtils.broadcastIpRules(ssc, "E:\\mrdata\\exam\\input\\ip.txt")
    ...
    kafkaStream.foreachRDD { kafkaRDD =>
      //判断当前的kafkaStream中的RDD是否有数据,银锭要记得写
      if(!kafkaRDD.isEmpty()) {
        ...
        //计算成交总金额
        CalculateUtil.productSum(lines)

        //计算商品分类金额
        CalculateUtil.calculateItem(lines)

        //计算区域成交金额
       CalculateUtil.calculateZone(lines, broadcastRef)
